...
if __name__ == "__main__":
    ...
    with open("./HAD-captions.json", "r") as f:
        inputs = json.load(f)
        inputs = [x for x in inputs if x["split"] == "conv"]
    ...
    When you answer the question, never mention given description directly, such as veclocity, steering angle, and time.
    ...
    Create a natural conversation dataset as if real people were talking!
    ...
    with open(
        f"HAD-instruct-conversation-{CURRENT_DATETIME}.jsonl", "w", encoding="utf-8"
    ) as main_file:
    ...
    with open(f"HAD-instruct-conversation.json", "w", encoding="utf-8") as f:
        json.dump(responses, f, ensure_ascii=False, indent=2)
